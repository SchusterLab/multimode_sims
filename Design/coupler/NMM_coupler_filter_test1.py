...
def cpw2cps_v1(structure, xpos, ypos, shape):
    c = structure
    pin_ref = c.s2.last
    ...
    # draw launcher part
    CPWStraight(c.s2, initial_length, pinw=0, gapw=shape['ground'])
    CPWStraight(c.s2, shape['gap2ground'], pinw=shape['launcher_width']*2, gapw=shape['ground']-shape['launcher_width'])
    ref1 = c.s2.last
    CPWStraight(c.s2, shape['launcher_length'], pinw=0,
                gapw=shape['launcher_width']/2)
    CPWLinearTaper(c.s2, shape['launcher_taper'], 0, 0, shape['launcher_width']/2, shape['cpw_pin'] / 2)
    c.s2.last = (ref1[0], ref1[1])
    CPWStraight(c.s2, shape['launcher_length'], pinw=shape['launcher_width']*2,
                gapw=shape['ground']-shape['launcher_width'])
    CPWLinearTaper(c.s2, shape['launcher_taper'], shape['launcher_width']*2, cpw_gap*2+shape['cpw_pin'], shape['ground']-shape['launcher_width'], shape['cpw_ground_width'])

    # draw port 4 section
    ref2 = c.s2.last
    CPWStraight(c.s2, shape['scpw_length'], pinw=0,
                gapw=shape['cpw_pin'] / 2)
    c.s2.last = (ref2[0], ref2[1])
    CPWStraight(c.s2, shape['scpw_length'], pinw=cpw_gap*2+shape['cpw_pin'],
                gapw=shape['cpw_ground_width'])

    ref2 = c.s2.last
    CPWStraight(c.s2, shape['cpw_ground4_length'], pinw=0,
                gapw=shape['cpw_pin'] / 2)
    c.s2.last = (ref2[0], ref2[1])
    CPWStraight(c.s2, shape['cpw_ground4_length'], pinw=cpw_gap * 2 + shape['cpw_pin'],
                gapw=shape['cpw_ground4'])
    # tapered section
    ref3 = c.s2.last
    CPWStraight(c.s2, shape['cpw_ground4_length']/np.sqrt(3), pinw=0,
                gapw=shape['cpw_pin'] / 2)
    port2_ref = ref3
    ...
    CPWLinearTaper(c.s2, shape['cpw_pin'] / 2 * np.sqrt(3), 0,
                   ...
                   shape['cpw_pin'] / 2, 0)
    port3_ref = c.s2.last
    c.s2.last = (ref3[0], ref3[1])
    CPWLinearTaper(c.s2, shape['cpw_ground4_length']/np.sqrt(3), cpw_gap * 2 + shape['cpw_pin'], cpw_gap * 2 + shape['cpw_pin'],
                   shape['cpw_ground4'], 0)
    ...
    ref4 = c.s2.last
    CPWStraight(c.s2, shape['cps_port2_length']+shape['cpw_ground_width'] / np.sqrt(3)*2, pinw=0,
                gapw=l / 2)
    c.s2.last = ref4
    CPWStraight(c.s2, shape['cps_port2_length']+shape['cpw_ground_width'] / np.sqrt(3)*2, pinw=l+shape['cpw_gap']*2,
                gapw=shape['cpw_ground_width'])
...
def multimode_pad(structure, xpos, ypos, shape):
    c = structure
    pin_ref = c.s2.last
    pin_direction = c.s2.last_direction
    ################################################
    ...
    # CPWBendNew(c.s2, angle=-90, pinw=0, gapw=10 / 2, radius=15,
    ...
    curve_corner(c.s2, save1[0]+shape['left_padx']-for_diff, save1[1] + shape['left_pady'] / 2, curve * 20, 1)
    curve_corner(c.s2, save1[0]+shape['left_padx']-for_diff, save1[1] - shape['left_pady'] / 2, curve * 20, 2)
...
def multimode_coupler(structure, xpos, ypos, shape):
    c = structure
    pin_ref = c.s2.last
    pin_direction = c.s2.last_direction
    ################################################
    ...
    section_length = 940 - 85 + 320 - 120 + 100+25+25+40
...
def draw_bridge_junction(structure, d, junc_correction, flip, type=0):
    c = structure
    ## alignment boxes for e-beam
    ...
    if draw_layer1 == True:
        ...
        multimode_coupler(c, reff[0], reff[1], shape1)
        xpos = 5450 / 2.
        ypos = 46000 - 17 + 3 + 890
        multimode_pad(c, xpos, ypos, shape1)
...
def output_wafer():
    ...
    junc_correction = {'width': 0.4, 'gap': 0.2}
    CHIPNAME = 'C1A'
    flip = False
    ...
    draw_bridge_junction(c0, d, junc_correction, flip, type=1)
    ...
    c1 = Chip(CHIPNAME, author=author, size=m.chip_size, mask_id_loc=(5800, 6430),
              chip_id_loc=(100, 100), textsize=(70, 70), two_layer=two_layer, solid=solid)
    draw_bridge_junction(c1, d, junc_correction, flip, type=1)

    # --------------------------#
    junc_correction = {'width': 0.45, 'gap': 0.2}
    flip = False
    CHIPNAME = 'C1C'
    ...
    draw_bridge_junction(c2, d, junc_correction, flip, type=1)

    # --------------------------#
    junc_correction = {'width': 0.5, 'gap': 0.2}
    flip = False
    CHIPNAME = 'C1D'
    c3 = Chip(CHIPNAME, author=author, size=m.chip_size, mask_id_loc=(5800, 6430),
              chip_id_loc=(200, 100), textsize=(70, 70), two_layer=two_layer, solid=solid)
    draw_bridge_junction(c3, d, junc_correction, flip, type=1)

    # --------------------------#
    junc_correction = {'width': 0.55, 'gap': 0.2}
    flip = False
    CHIPNAME = 'C1E'
    c4 = Chip(CHIPNAME, author=author, size=m.chip_size, mask_id_loc=(5800, 6430),
              chip_id_loc=(200, 100), textsize=(70, 70), two_layer=two_layer, solid=solid)
    draw_bridge_junction(c4, d, junc_correction, flip, type=1)

    # --------------------------#
    junc_correction = {'width': 0.6, 'gap': 0.2}
    flip = False
    CHIPNAME = 'C1F'
    c5 = Chip(CHIPNAME, author=author, size=m.chip_size, mask_id_loc=(5800, 6430),
              chip_id_loc=(200, 100), textsize=(70, 70), two_layer=two_layer, solid=solid)
    draw_bridge_junction(c5, d, junc_correction, flip, type=1)

    # --------------------------#
    junc_correction = {'width': 0.13, 'gap': 0.2}
    flip = False
    CHIPNAME = 'C1G'
    c6 = Chip(CHIPNAME, author=author, size=m.chip_size, mask_id_loc=(5800, 6430),
              chip_id_loc=(200, 100), textsize=(70, 70), two_layer=two_layer, solid=solid)
    draw_bridge_junction(c6, d, junc_correction, flip, type=0)
    ...
    c7 = Chip(CHIPNAME, author=author, size=m.chip_size, mask_id_loc=(5800, 6430),
              chip_id_loc=(200, 100), textsize=(70, 70), two_layer=two_layer, solid=solid)
    draw_bridge_junction(c7, d, junc_correction, flip, type=0)

    # --------------------------#
    junc_correction = {'width': 0.12, 'gap': 0.2}
    flip = False
    CHIPNAME = 'C1I'
    c8 = Chip(CHIPNAME, author=author, size=m.chip_size, mask_id_loc=(5800, 6430),
              chip_id_loc=(200, 100), textsize=(70, 70), two_layer=two_layer, solid=solid)
    draw_bridge_junction(c8, d, junc_correction, flip, type=0)
    ...
    c9 = Chip(CHIPNAME, author=author, size=m.chip_size, mask_id_loc=(5800, 6430),
              chip_id_loc=(200, 100), textsize=(70, 70), two_layer=two_layer, solid=solid)
    draw_bridge_junction(c9, d, junc_correction, flip, type=1)
    ...
    c10 = Chip(CHIPNAME, author=author, size=m.chip_size, mask_id_loc=(5800, 6430),
               chip_id_loc=(200, 100), textsize=(70, 70), two_layer=two_layer, solid=solid)
    draw_bridge_junction(c10, d, junc_correction, flip, type=1)

    # --------------------------#
    junc_correction = {'width': 0.35, 'gap': 0.2}
    flip = False
    CHIPNAME = 'C1L'
    c11 = Chip(CHIPNAME, author=author, size=m.chip_size, mask_id_loc=(5800, 6430),
               chip_id_loc=(200, 100), textsize=(70, 70), two_layer=two_layer, solid=solid)
    draw_bridge_junction(c11, d, junc_correction, flip, type=1)

    # --------------------------#
    junc_correction = {'width': 0.25, 'gap': 0.2}
    flip = False
    CHIPNAME = 'C1M'
    c12 = Chip(CHIPNAME, author=author, size=m.chip_size, mask_id_loc=(5800, 6430),
               chip_id_loc=(200, 100), textsize=(70, 70), two_layer=two_layer, solid=solid)
    draw_bridge_junction(c12, d, junc_correction, flip, type=1)
    ...
    c13 = Chip(CHIPNAME, author=author, size=m.chip_size, mask_id_loc=(5800, 6430),
               chip_id_loc=(200, 100), textsize=(70, 70), two_layer=two_layer, solid=solid)
    draw_bridge_junction(c13, d, junc_correction, flip, type=0)
