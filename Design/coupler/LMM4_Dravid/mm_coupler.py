...
class MultimodeCouplerPad(MMChipsBase):
    def __init__(self, structure, xpos, ypos, shape, short=False):
        super().__init__()
        self.structure = structure
        self.xpos = xpos
        self.ypos = ypos
...
class MultimodeCouplerChip(MMChipsBase):
    ...
    def draw_coupler_optical(self, balun_configs, coupler_configs, flux_line_bool = True, short = False):
        """
        ...
        # if short:
        #     #fill in the space between the squid loops
        #     print("Drawing short between SQUIDs")
        #     distance_between_squids = coupler_configs.get('squid_x', 60) +37
        #     size_y_squid = coupler_configs.get('squid_y', 22)
        #     c.s2.last_direction = 0
        #     c.s2.last = (xpos-(distance_between_squids/2), ypos)
        #     CPWStraight(c.s2, distance_between_squids, pinw=0, gapw=size_y_squid/2)
