...
class MultimodeCoupler(MMChipsBase): 
    ...
    def __init__(self, structure, xpos, ypos, loop_configs = None, 
                 junction_configs = None, optical_structure=0, jj = False):
        super().__init__()
        ...
        self.jj = jj
        # short open or jj 
        if junction_configs is not None: 
            self.short = junction_configs['short']
            self.open = junction_configs['open']
            if self.short or self.open: 
                self.jj = False
    ...
    def draw_djj_frame(self):
        ...
        if short: 
            ...
            CPWStraight(c.s2.pin_layer, total_junction_length, pinw=0,
                        gapw=djj['bar_width']/2) 
            #undercut
            c.s2.gap_layer.last_direction = 0
            c.s2.gap_layer.last = (
            xpos - total_junction_length / 2, ypos - djj['bar_width'] / 2) # left edge
            CPWStraight(c.s2.gap_layer, total_junction_length, pinw=djj['bar_width'], gapw=gap_thin)

        elif self.open: 
            #undercut
            c.s2.gap_layer.last_direction = 90
            c.s2.gap_layer.last = (xpos, ypos - djj['bar_width']) # center 
            CPWStraight(c.s2.gap_layer, djj['bar_width'], pinw=total_junction_length- 2*gap_thin, gapw=gap_thin)
        elif jj:
...
class MultimodeCouplerPad(MMChipsBase):
    ...
    def draw(self, squid_movey = 813-250-1400):
        c = self.structure
        shape = self.shape
        xpos = self.xpos
        ypos = self.ypos
        ...
        ################################################
        c.s2.last = (xpos, ypos)
        movey = squid_movey # coupler center position
        curve = 5
        ...
        thin_bar_length =shape['thin_bar_length']
        fat_bar_length = shape['bar_length'] - gap_squid / 2 - thin_bar_length
        width_long_left_pad = shape['width_long_left_pad'] # this is the width of long section of left pad (length in terms of drawn CPW)
...
class MultimodeFluxLine(MMChipsBase):
    ...
    def draw(self):
        ...
        loop_curve = 15
        CPWStraight(c.s2, shape['length_pad_to_loop'] + free_y, pinw=sgap, gapw=width)
...
class MultimodeCouplerChip(MMChipsBase):
    ...
    def draw_alignment_marks(self, draw_marks = False, args = None):
        '''
        args: upper_xpos, upper_ypos, lower_xpos, lower_ypos
        '''
        c = self.structure
        ## alignment boxes for e-beam
        # self.draw_launchers(c, self.d, exclude=[2, 3, 8])
        if draw_marks:
            self.draw_square_alignment_marks(c, **args)

    def draw_square_alignment_marks(self, structure, upper_xpos=None, upper_ypos=None, lower_xpos=None, lower_ypos=None):
        c = structure
        shift_y = 0
        align_box = 80
        print('upper ypos:', upper_ypos)
        
        # Use provided parameters or default values
        left_x = upper_xpos[0] if upper_xpos is not None else 200
        right_x = upper_xpos[1] if upper_xpos is not None else 5250
        upper_y = upper_ypos if upper_ypos is not None else 2100 + 50500
        lower_y = lower_ypos if lower_ypos is not None else 700
        
        # Upper alignment marks
        c.s2.last = (left_x, upper_y + align_box / 2 + shift_y)
        CPWStraight(c.s2, align_box, pinw=0, gapw=align_box / 2)
        c.s2.last = (right_x, upper_y + align_box / 2 + shift_y)
        CPWStraight(c.s2, align_box, pinw=0, gapw=align_box / 2)

        # Lower alignment marks
        c.s2.last = (left_x if lower_xpos is None else lower_xpos[0], lower_y + align_box / 2)
        CPWStraight(c.s2, align_box, pinw=0, gapw=align_box / 2)
        c.s2.last = (right_x if lower_xpos is None else lower_xpos[1], lower_y + align_box / 2)
        CPWStraight(c.s2, align_box, pinw=0, gapw=align_box / 2)
        c.s2.last = (c.size[0] / 2 - 1900, c.size[1] - 600)

        # Larger alignment marks
        align_box = 500
        c.s2.last = (left_x, upper_y + align_box / 2 + shift_y)
        CPWStraight(c.s2, align_box, pinw=0, gapw=align_box / 2)
        c.s2.last = (right_x, upper_y + align_box / 2 + shift_y)
    ...
    def draw_cross_alignment_marks(self, structure, flip):
        c = structure

        def draw_cross(center, arm_length=200, arm_width=20, square_size=80):
            """Draw a cross centered at `center` with a central square."""
            cx, cy = center

            # central 80 µm square
            c.s2.last_direction = 0
            c.s2.last = (cx - square_size / 2, cy)
            CPWStraight(c.s2, square_size, pinw=0, gapw=square_size / 2)

            # horizontal arms
            c.s2.last_direction = 0
            c.s2.last = (cx - arm_length / 2, cy )
            CPWStraight(c.s2, arm_length, pinw=0, gapw=arm_width / 2)
            # c.s2.last = (cx - arm_length / 2, cy + arm_width / 2)
            # CPWStraight(c.s2, arm_length, pinw=0, gapw=arm_width / 2)

            # vertical arms
            c.s2.last_direction = 90
            c.s2.last = (cx, cy - arm_length / 2)
            CPWStraight(c.s2, arm_length, pinw=0, gapw=arm_width / 2)
            # c.s2.last = (cx + arm_width / 2, cy - arm_length / 2)
            # CPWStraight(c.s2, arm_width, pinw=0, gapw=arm_length / 2)

        if flip:
            # define positions for flipped case
            centers = [
                (150, 2100 + 18100 - 8300),
                (5350, 2100 + 18100 - 8300),
                (150, 2100 + 18100 - 700),
                (5350, 2100 + 18100 - 700),
            ]
        else:
            # define positions for normal case
            centers = [
                (200, 2100 + 50500),
                (5250, 2100 + 50500),
                (200, 700),
        ...
        # draw cross at each position
        for center in centers:
            draw_cross(center, square_size=80)

        # reset cursor position
        c.s2.last = (c.size[0] / 2 - 1900, c.size[1] - 600)


    def draw_coupler_optical(self, balun_configs, coupler_configs, flux_line_bool = True):
        """
        ...
        # Draw coupler pad
        xpos = coupler_configs.get('xpos', 5450 / 2.)
        ypos = coupler_configs.get('ypos', 46000 - 17 + 3 + 890)
        coupler_pad = MultimodeCouplerPad(structure=c, xpos=xpos, ypos=ypos, shape=coupler_configs)#, short = short)
        coupler_pad.draw(squid_movey=coupler_configs.get('squid_movey', -837))
    ...
    def draw_coupler_ebeam(self, loop_configs, junction_configs, junc_correction):
        ...
        # Draw SQUID loop
        xpos = junction_configs.get('xpos', 5450 / 2.)
        ypos = junction_configs.get('ypos', 46000 - 17 + 3 + 64 +1) # this is the center of top junction
        ...
        buffer_length = 0.2 #between junctions 
        djj['jj_width'] *= djj['num_juncs']  # make the pin wider for reducing Ej when multiple junctions

        leftmost_origin = (xpos - djj['num_juncs'] * (buffer_length + total_junction_length) / 2 + total_junction_length / 2, ypos)
        for i in range(djj['num_juncs']):
            xpos_shifted = leftmost_origin[0] + i * (buffer_length + total_junction_length)
            coupler.coords['origin'] = (xpos_shifted, leftmost_origin[1])
            coupler.draw_djj_frame()
            # Draw Dolan bridge junction (bottom)
            shifted_ypos = ypos - loop_configs['y'] - loop_configs['width']
            coupler.coords['origin'] = (xpos_shifted, shifted_ypos)
            coupler.draw_djj_frame()
