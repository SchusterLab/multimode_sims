...
class MultimodeQubit(MMChipsBase):
    ...
    def draw_ebeam(self):
        ...
        total_junction_length = shape['taper_length1'] + shape['thin_bar_length1'] + shape['taper_length2'] + shape['thin_bar_length2'] + shape['jj_gap'] + shape['thin_bar_length'] + shape['top_bar_taper_length']
        ...
        if self.open: 
            # go to top bar 
            c.s2.pin_layer.last = (xpos, ypos +shape['bar_length'] +  total_junction_length)

        elif self.short: 
            CPWStraight(c.s2.pin_layer, total_junction_length, pinw=0, gapw=shape['bar_width'] / 2)
        ...
        if self.open: 
            # the top edge of bottom bar
            CPWStraight(c.s2.gap_layer, uc_width_pads, pinw =0, gapw = uc_width_pads + shape['bar_width'] / 2)
            # the bottom edge of top bar 
            c.s2.gap_layer.last = (xpos, ypos +shape['bar_length']+ total_junction_length - uc_width_pads)
            CPWStraight(c.s2.gap_layer, uc_width_pads, pinw =0, gapw = uc_width_pads + shape['bar_width'] / 2)


        elif self.short:
            CPWStraight(c.s2.gap_layer, total_junction_length, pinw=shape['bar_width'], gapw=uc_width_pads)
