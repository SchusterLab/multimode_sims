...
class WaferMask(sdxf.Drawing):
    ...
    def __init__(self, name, diameter=50800., flat_angle=90., flat_distance=24100., wafer_padding=2000,
        ...
        #Writes the wafer shape
        if wafer_edge:
            self.append(sdxf.PolyLine(pts))

        if square_arr:
            self.chip_points = self.get_chip_points_square()
        else:
    ...
    def add_chip(self, chip, copies, label=False, savechip=True, save_folder =None):
        ...
        if savechip:
            if save_folder is None:
                return chip.save(fname=self.name + "-" + chip.name, maskid=self.name, chipid=chip.name, do_label=label)
            else:
                if save_folder[-1] != '\\':
                    save_folder += '\\'
                return chip.save(fname=save_folder + self.name + "-" + chip.name, maskid=self.name, chipid=chip.name, do_label=label)
                #return chip.save(fname=self.name + "-" + chip.name, maskid=self.name, chipid=chip.name, do_label = label)
    ...
    def get_chip_points_square(self):
        """Get insertion points for all of the chips in a square array"""
        max_cols = int((self.diameter - 2 * self.buffer) / self.die_size[0]) - 1
        max_rows = int((self.diameter - 2 * self.buffer) / self.die_size[1]) - 1
        print("Maximum number of rows=%d and cols=%d" % (max_rows, max_cols))
        #figure out offset for chips (centered on chip or between chips)
        xoffset = -max_cols / 2. * self.die_size[0]
        yoffset = -max_rows / 2. * self.die_size[1]
        ...
        for ii in range(max_rows):
            for jj in range(max_cols):
                pt = (xoffset + jj * self.die_size[0], yoffset + ii * self.die_size[1])
                if self.die_inside(pt):
                    pass # since it results in False for corner points   - Tanay
                chip_points.append(translate_pt(pt, (self.dicing_border / 2., self.dicing_border / 2.)))
        print("Room for %d chips on wafer." % chip_points.__len__())
        return chip_points
...
class Chip(sdxf.Block):
    ...
    def label_chip(self, drawing, maskid, chipid, author, offset=(0, 0), textsize=None):
        ...
        if layer != 'GAP': # i don't want chip label in ebeam
            AlphaNumText(drawing, chipid, textsize, translate_pt(self.chip_id_loc, offset), layer=layer)
...
class Launcher:
    def __init__(self, structure, flipped=False, pad_length=400, taper_length=250, pad_to_length=500, pinw=None,
                 gapw=None, launcher_pinw=300, launcher_gapw=150): # pad_length=250
        s = structure
        ...
        self.pinw = launcher_pinw #150
        self.gapw = launcher_gapw #75
        if not flipped:
            #input launcher
